...
Skills are installed in `.codex/skills/` at the project level. Skills use the naming pattern `aico-{employee}-{skill}`.

To use a skill explicitly, type `$aico-{employee}-{skill}` or let Codex auto-select based on your task.

### Tool Mapping (Claude Code → Codex)

When skills reference Claude Code tools, use these Codex equivalents:

| Claude Code Tool                | Codex Equivalent   | Notes                   |
| ------------------------------- | ------------------ | ----------------------- |
| `TodoWrite`                     | `update_plan`      | Task tracking           |
| `Task` (subagents)              | Direct execution   | Subagents not available |
| `Read`, `Write`, `Edit`, `Bash` | Native Codex tools | Same functionality      |

### Custom Prompts (Commands)

Codex custom prompts are installed globally at `~/.codex/prompts/`. Use them with `/prompts:aico.{employee}.{command}`:

| Command                         | Usage                           |
| ------------------------------- | ------------------------------- |
| `/prompts:aico.pm.init`         | Initialize PM constitution      |
| `/prompts:aico.pm.plan`         | Create product plan             |
| `/prompts:aico.pm.clarify`      | Clarify requirements            |
| `/prompts:aico.frontend.init`   | Initialize frontend constraints |
| `/prompts:aico.frontend.design` | Generate design specs           |
| `/prompts:aico.frontend.plan`   | Create frontend plan            |
| `/prompts:aico.frontend.tasks`  | Break down frontend tasks       |
| `/prompts:aico.backend.init`    | Initialize backend constraints  |
| `/prompts:aico.backend.plan`    | Create backend plan             |
| `/prompts:aico.backend.tasks`   | Break down backend tasks        |
...
6. **Tool mapping errors**: Remember to use Codex equivalents for Claude Code tools.
